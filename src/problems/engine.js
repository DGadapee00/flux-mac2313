 ...
 */
// Calc 3 has no SI constants and no dimensional analysis. The grader still evaluates both
// expressions at 8 seeded points; typed gradients do not need an AST differentiator.
...
const GREEK = { 'π': 'pi', 'θ': 'th', 'φ': 'phi', 'ρ': 'rho', 'λ': 'lam', 'α': 'alpha', 'ω': 'omega' };
 ...
 */
export const GLYPH = {
  pi: 'π', th: 'θ', phi: 'φ', rho: 'ρ', lam: 'λ', alpha: 'α', omega: 'ω', e: 'e',
};

const TEX_SYM = {
  pi: '\\pi', th: '\\theta', phi: '\\varphi', rho: '\\rho',
  lam: '\\lambda', alpha: '\\alpha', omega: '\\omega',
};
...
/**
 * Evaluate a typed expression at a scope (the lab's current point, plus any sampled parameters).
 * This is the Calc 3 stand-in for the units line: what the formula *equals* at P, while you type.
 */
export function evalAtScope(input, part, scope) {
  const fn = compile(input, part.vars, part.alias);
  return fn(scope);
}
