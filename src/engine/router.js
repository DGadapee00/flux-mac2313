...
/**
 * Parse `#/ch2/gradient`, `#/gradient`, or empty, plus an optional problem query:
 * `#/ch2/gradient?p=ch2.grad.point&s=7` opens that problem with seed 7.
 */
export function parseHash() {
  ...
  if (!a) return { examId: 'ch2', labId: 'gradient', ...problem };
  if (LAB_META[a] && !b) {
 ...
 * `replace` is for boot, where we only normalize the URL (e.g. `#/` → `#/ch2/gradient`).
 */
