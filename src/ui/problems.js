...
import { instance, render, grade, expected, sig, withinTol, compile, evalAtScope, exprToTex, GLYPH } from '../problems/engine.js';
...
export function createPractice(api) {
  ...
  function paletteHTML(part, i) {
    ...
    const consts = ['pi'].filter((c) => !part.vars.includes(c)).map((c) => chip(GLYPH[c] || c));
    /*
  ...
  function parseMessage(part, value) {
    ...
    const P = s?.probe;
    const cur = st.cur;
    if (!P || !cur) return { cls: '', html: '' };
    try {
      const sc = { ...(cur.inst?.$ || {}), x: P.x, y: P.y };
      const v = evalAtScope(value, part, sc);
      if (!Number.isFinite(v)) return { cls: '', html: '' };
      return { cls: 'ok', html: `at P: ${esc(sig(v))}` };
    } catch {
      return { cls: '', html: '' };
    }
  }
  ...
  function problemHTML(cur) {
    ...
    return `${head}
      ...
      <h2 class="pb-h">${mathProse(tpl.title)}</h2>
    ...
    const items = ex.items
      .map((it, i) => {
        ...
        return `<button type="button" class="pb-item" data-review="${i}">
          ...
          <span class="pb-item-title">${i + 1}. ${mathProse(tpl.title)}</span>
