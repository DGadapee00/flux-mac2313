...
export class NumberPlane {
  constructor(half = GRID_HALF) {
    ...
    this.nameB = this.makeName('y', 0, half + 0.35, 0);
    this.nameOff = this.makeName('z', 0, 0, 0);
    this.nameOff.position.set(0, 5.2, 0);
    this.group.add(this.nameOff);

    this.scale = 0;
    this.planeId = '';
    this.setPlane('xyfloor');
    this.setScale(1);
  }
  ...
  setPlane(id) {
    ...
    // xyfloor: domain on the floor labeled x, y; height is z. xz keeps the physics labels.
    const b = id === 'xz' ? 'z' : 'y';
    const off = id === 'xz' ? 'y' : 'z';
